...
public class KeyCollectible : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Player") && !isCaptured)
        {
            ...
            // Update the key was collected on Player Movement.
            collision.gameObject.GetComponent<PlayerMovement>().UnlockKey();

            StartCoroutine(nameof(CaptureKey));            
        }
    }

    private void Movement()
    {
        float distSqr = (transform.position - movePos[idxPos]).sqrMagnitude;
        ...
        if (distSqr <= 0.0001f)
        {
            ...
            transform.position = Vector3.Lerp(transform.position, movePos[idxPos], t);
    }
    ...
    private IEnumerator CaptureKey()
    {        
